    ...
    def isIn(self, point: npvector3):
        for i in range(3):
            ...
            if (self.max._vec3[i] < point._vec3[i]):
                return False
        #
        # for min_coord, max_coord, point_coord in zip(self.min._vec3, self.max._vec3, point._vec3):
        #     if (min_coord > point_coord) or (max_coord < point_coord):
        #         return False
        return True

    def isIn_new(self, point: npvector3):
        vec_isIn = np.greater(self.min._vec3, point._vec3)
        vec_isIn2 = np.greater(point._vec3, self.max._vec3)

        if (False in vec_isIn) or (False in vec_isIn2):
            return False
        else:
            return True
